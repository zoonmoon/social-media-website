...
export async function DELETE(request, { params }) {
    ...
    try {
        ...
        let query = `
            DELETE FROM dynamic_pages WHERE slug = '${slug}'
        `;
        ...
        if (result.affectedRows > 0) {
            return new Response(JSON.stringify({ success: true, message: 'Dynamic Page deleted successfully' }));
        } else {
            return new Response(JSON.stringify({ success: false, message: 'No Dynamic found with that slug' }));
        }
